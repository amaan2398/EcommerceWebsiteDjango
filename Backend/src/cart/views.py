...
def cart(request):
    a = ['','active','']
    cdata = Cart.objects.filter(customer_id=request.user.id)
    data = []
    tamount = 0
    for i in cdata:
    ...
    cid = request.user.id
    cdata = Cart.objects.filter(customer_id=cid)
...
def addtocart(request,id):
    ...
    if Cart.objects.filter(customer_id= cid,product_id=id).exists():
        a = Cart.objects.get(customer_id= cid,product_id=id)
        a.product_quantity += 1
        a.save()
        del a
    else:
        a = Cart(customer_id=cid,product_id=id,product_quantity=1)
        a.save()
        del a
    return redirect("/")
...
def addrm_pro_qut(request,id,v):
    ...
    if Cart.objects.filter(customer_id= cid,product_id=id)[0].product_quantity == 0:
        a = Cart.objects.filter(customer_id= cid,product_id=id)
        a.delete()
